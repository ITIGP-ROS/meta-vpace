/* aes_cmac.c — AES-128 (encrypt-only) + CMAC (RFC 4493). See aes_cmac.h. */
#include "aes_cmac.h"
#include <string.h>

/* ---------------------------- AES-128 core ---------------------------- */

static const uint8_t sbox[256] = {
 };

static const uint8_t rcon[11] = { 0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36 };

static uint8_t xtime(uint8_t x) { return (uint8_t)((x << 1) ^ (((x >> 7) & 1) * 0x1b)); }

static void key_expansion(const uint8_t key[16], uint8_t rk[176]) {
    memcpy(rk, key, 16);
    for (int i = 16; i < 176; i += 4) {
        uint8_t t[4];
        memcpy(t, rk + i - 4, 4);
        if (i % 16 == 0) {
            uint8_t tmp = t[0];
            t[0] = (uint8_t)(sbox[t[1]] ^ rcon[i / 16]);
            t[1] = sbox[t[2]];
            t[2] = sbox[t[3]];
            t[3] = sbox[tmp];
        }
        for (int j = 0; j < 4; j++) rk[i + j] = (uint8_t)(rk[i - 16 + j] ^ t[j]);
    }
}

static void aes128_encrypt(const uint8_t rk[176], const uint8_t in[16], uint8_t out[16]) {
    uint8_t s[16];
    memcpy(s, in, 16);
    for (int i = 0; i < 16; i++) s[i] ^= rk[i];                 /* AddRoundKey 0 */

    for (int round = 1; round <= 10; round++) {
        for (int i = 0; i < 16; i++) s[i] = sbox[s[i]];        /* SubBytes */

        uint8_t t;                                             /* ShiftRows */
        t = s[1];  s[1]=s[5];  s[5]=s[9];   s[9]=s[13];  s[13]=t;
        t = s[2];  s[2]=s[10]; s[10]=t;     t=s[6]; s[6]=s[14]; s[14]=t;
        t = s[15]; s[15]=s[11];s[11]=s[7];  s[7]=s[3];   s[3]=t;

        if (round != 10) {                                     /* MixColumns */
            for (int c = 0; c < 4; c++) {
                uint8_t *col = s + 4 * c;
                uint8_t a0=col[0], a1=col[1], a2=col[2], a3=col[3];
                col[0] = (uint8_t)(xtime(a0) ^ (xtime(a1)^a1) ^ a2 ^ a3);
                col[1] = (uint8_t)(a0 ^ xtime(a1) ^ (xtime(a2)^a2) ^ a3);
                col[2] = (uint8_t)(a0 ^ a1 ^ xtime(a2) ^ (xtime(a3)^a3));
                col[3] = (uint8_t)((xtime(a0)^a0) ^ a1 ^ a2 ^ xtime(a3));
            }
        }
        for (int i = 0; i < 16; i++) s[i] ^= rk[16 * round + i]; /* AddRoundKey */
    }
    memcpy(out, s, 16);
}

/* ------------------------------- CMAC -------------------------------- */

static void leftshift1(const uint8_t in[16], uint8_t out[16]) {
    uint8_t carry = 0;
    for (int i = 15; i >= 0; i--) {
        uint8_t b = in[i];
        out[i] = (uint8_t)((b << 1) | carry);
        carry = (uint8_t)((b >> 7) & 1);
    }
}

void aes128_cmac(const uint8_t key[16], const uint8_t *msg, size_t len, uint8_t mac[16]) {
    uint8_t rk[176];
    key_expansion(key, rk);

    /* Subkeys K1, K2 (RFC 4493 §2.3). Rb = 0x87 for the 128-bit block. */
    uint8_t L[16], K1[16], K2[16];
    const uint8_t zero[16] = {0};
    aes128_encrypt(rk, zero, L);
    leftshift1(L, K1);  if (L[0]  & 0x80) K1[15] ^= 0x87;
    leftshift1(K1, K2); if (K1[0] & 0x80) K2[15] ^= 0x87;

    size_t n = (len + 15) / 16;
    int complete;
    if (n == 0) { n = 1; complete = 0; }
    else        { complete = (len % 16 == 0); }

    uint8_t X[16] = {0}, Y[16], M_last[16];

    for (size_t i = 0; i + 1 < n; i++) {
        for (int j = 0; j < 16; j++) Y[j] = (uint8_t)(X[j] ^ msg[16 * i + j]);
        aes128_encrypt(rk, Y, X);
    }

    if (complete) {
        for (int j = 0; j < 16; j++) M_last[j] = (uint8_t)(msg[16 * (n - 1) + j] ^ K1[j]);
    } else {
        size_t rem  = len % 16;         /* 0 only when len == 0 */
        size_t base = 16 * (n - 1);
        for (size_t j = 0; j < 16; j++) {
            uint8_t b = (j < rem) ? msg[base + j] : (j == rem ? 0x80 : 0x00);
            M_last[j] = (uint8_t)(b ^ K2[j]);
        }
    }

    for (int j = 0; j < 16; j++) Y[j] = (uint8_t)(X[j] ^ M_last[j]);
    aes128_encrypt(rk, Y, mac);
}
